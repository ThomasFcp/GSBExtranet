...
class PdoGsb{   		
      	...
      	private static $user='Admin_GSB' ;    		
      	private static $mdp='gtejNFcuQIIFXLX5' ;	
...
{
   $hmdp = PASSWORD_HASH($mdp, PASSWORD_BCRYPT);
   ...
   $bv2 = $pdoStatement->bindValue(":leMdp", $hmdp);
   $execution = $pdoStatement->execute();
   return $execution;
}
...
public static function rectificationSansMdp($nom,$prenom, $id){
    $pdo = PdoGsb::$monPdo;
    $pdoStatement=$pdo->prepare("UPDATE medecin SET nom =:leNom, prenom =:lePrenom WHERE id=:lId");
    $bv1 = $pdoStatement->bindValue(':leNom', $nom);
    $bv2 = $pdoStatement->bindValue(':lePrenom', $prenom);
    $bv3 = $pdoStatement->bindValue(':lId', $id);
    $executionok=$pdoStatement->execute();
    $resultatRequete=$pdoStatement->fetch();
}
public static function rectificationAvecMdp($nom,$prenom, $mdp, $id){
    $pdo = PdoGsb::$monPdo;
    $pdoStatement=$pdo->prepare("UPDATE medecin SET nom =:leNom, prenom =:lePrenom, motDePasse =:leMDP WHERE id=:lId");
    $bv1 = $pdoStatement->bindValue(':leNom', $nom);
    $bv2 = $pdoStatement->bindValue(':lePrenom', $prenom);
    $bv3 = $pdoStatement->bindValue(':leMDP', $mdp);
    $bv4 = $pdoStatement->bindValue(':lId', $id);
    $executionok=$pdoStatement->execute();
    $resultatRequete=$pdoStatement->fetch();
}
...
function enregistreDeconnexion($id){
    $pdoStatement = PdoGsb::$monPdo->prepare("UPDATE historiqueconnexion
            SET dateFinLog=now() WHERE :leMedecin=idMedecin AND dateFinLog IS NULL ");
    $bv1 = $pdoStatement->bindValue(':leMedecin', $id);
    $execution = $pdoStatement->execute();
    return $execution;
}
